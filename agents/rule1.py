...
def agent(obs):
    ...
    def ball_landing(ball, ball_direction):
        ...
        time = np.sqrt(
            start_speed ** 2 / gravity ** 2 - 2 / gravity * (end_height - start_height)) + start_speed / gravity
    ...
    if obs['ball_owned_player'] == obs['active'] and obs['ball_owned_team'] == 0:
        ...
        if obs['game_mode'] == GameMode.KickOff:
            return sticky_check(Action.ShortPass, Action.Top) if controlled_player_pos[1] > 0 else sticky_check(
                Action.ShortPass, Action.Bottom)
        ...
        if obs['game_mode'] == GameMode.FreeKick:
            if controlled_player_pos[0] > goal_threshold:
                ...
                if abs(controlled_player_pos[1]) < 0.3:
                    return sticky_check(Action.Shot, Action.TopRight) if controlled_player_pos[1] > 0 else sticky_check(
                        Action.Shot, Action.BottomRight)
                return sticky_check(Action.HighPass, Action.Top) if controlled_player_pos[1] > 0 else sticky_check(
                    Action.HighPass, Action.Bottom)
